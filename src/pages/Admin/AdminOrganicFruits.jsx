...
import { addOrganicFruits, getOrganicFruits,deleteOrganicFruits,editOrganicFruits } from "../../api/Api";
import {toast} from 'sonner'
//import BasicFruits from "../BasicFruits";
const AdminOrganicFruits = () => {
  //null -> products[] | Store the data
  const [organicfruits, setOrganicFruits] = useState(null);
  ...
  const fetchData = async () => {
    try {
      const res = await getOrganicFruits();
      if (res.status === 200) {
        console.log(res.data);
        setOrganicFruits(res.data);
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await addOrganicFruits(organicfruits)
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editOrganicFruits(product, currentProduct._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteOrganicFruits(id);
      if (response.status === 200) {
  ...
  if (!organicfruits || organicfruits.length === 0) {
    return (
      <>
        <div className="w-screen h-[90vh] flex flex-col justify-center items-center">
  ...
  return (
    <div className="w-full flex flex-col justify-start items-start">
      <div className="w-full flex flex-row justify-between items-center my-4 shadow-md rounded-md p-1 border">
        <AdminPageHeader title="Organic Fruits" />
        <button
      ...
      <table className="w-full h-full border-collapse border shadow-lg rounded-md">
        ...
        <tbody>
          {organicfruits.map((product, index) => (
            <tr key={index}>
          ...
          <div className="absolute top-0 left-0 z-50 h-screen w-screen flex justify-center items-center bg-black/40 ">
            <div className="h-[65%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md">
              <div className="h-full w-full flex flex-col justify-center items-center text-lg font-semibold">
                ...
                <form
                  className="h-[70%] w-[80%] flex flex-col justify-center items-center gap-6"
                  onSubmit={handleAdd}
                >
                  <input
                    ref={titleRef}
            ...
            <div className='h-[70%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                ...
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-6' onSubmit={handleEdit}>
                  <input ref={titleRef} type="text" name="" id="name" placeholder='Title' defaultValue={currentProduct.title} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-2 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required autoFocus />
                  <input ref={imgRef} type="text" name="" id="img" placeholder='Image URL' defaultValue={currentProduct.img} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-2 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={priceRef} type="number" name="" id="price" placeholder='Price' defaultValue={currentProduct.price} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-2 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
